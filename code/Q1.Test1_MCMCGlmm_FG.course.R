#QUESTION 1: Are relationships between traits conserved across phylogenies 
...
#1) FE vs all variance + error = R2m
...
stoic.trait <- list( c('nexc.m', "bodyn" ),
                    ...
                    c("bodyp", "bodyn", "bodyc"),
                    c("bodyc", "bodyp"), #based on the Allgeier et al. 2020 Scientific Reports
                    c("bodynp", "bodyc"),
                    c("bodycn","bodyp"),
                    c("bodycp","bodyn") )
...
for(i in 1:length(stoic.trait)){
   ...
   now <- data.frame(region = as.factor(m$region),
                     ...
                     fam = as.factor(m$family), #can't write thw whole word "family" because of the mcmcmglmm calls
   ...
   #Read the tree 
   sp.tree.full <- read.newick("/Users/jeallg/My Work Biatch/Fish Datasets/Caribbean+Pacific Fish Stoic/Evolution of fish pee/R input/actinopt_12k_treePL.tre") 
   ...
   #because there are repeated values for each species
   sp.tree<-drop.tip(sp.tree.full, setdiff(sp.tree.full$tip.label, sp.list))
   
   #see what is missing - right now, you get 183 species from your dataset
   if( length(setdiff(sp.list, sp.tree$tip.label)) > 0 ) { " FlUCK SpIT !"}
   ...
   #this ensures mass is not in the equation for excretion
   if(i == 1 | i == 2) {
     mods <- formula(paste(s.trait, "~"," -1 + FG + region + region:sn15 + region:sc13 ",stoic.preds)) } else {
   ...
   #isolate the parameter estimates and look at trace plots
   fixed <- paste(stoic.trait[[i]][1],"FE","pdf", sep = ".")
   pdf( file= paste("data/output_woPalmyra/plots/",fixed,sep = "" ) )
   plot(m1$Sol)
   
   dev.off()
   random <- paste(stoic.trait[[i]][1],"RE","pdf", sep = ".")
   pdf( file= paste("data/output_woPalmyra/plots/",random,sep = "" ))
   plot(m1$VCV)
   dev.off()
   ...
   #matrix of the conditional R2 of an intercept only model with CIs 
...
write.csv(data.frame(R2.glmm.mat), file = "data/output/AllStoicTraitModels_R2.glmm")
write.csv(Lambda.mat,file = "data/output/AllStoicTraitModels_P.Lambda")
write.csv(DIC.mat,file = "data/output/AllStoicTraitModels_DIC")
saveRDS(allstoic.params, file = "data/output/AllStoicTraitModels_ParamOutputs", 
